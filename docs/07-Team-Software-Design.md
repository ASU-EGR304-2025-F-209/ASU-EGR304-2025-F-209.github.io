...
## Version 2.0
**Hardware V2.0**
<u>Modular System Architecture</u>
...
<u>Improved Diagnostics & LED Feedback</u>
...
<u>State Machine Architecture</u>
The current system uses separate state machines that can become unsynchronized. Version 2.0 should use a centralized hierarchical state machine to coordinate all subsystems under one control. This prevents conflicting behaviors, ensures transitions only occur when conditions are valid, and allows state history tracking for easier debugging and recovery.

<u>Input Debounce and Validation</u>
The fingerpad input should include software debouncing through delays to prevent false triggers from switch noise. Adding input timeouts to clear incomplete password entries, max-length limits and detection of impossible input sequences could improve security and reliability.

<u>Configurable Alarm Timer</u>
Instead of a fixed delay, or a toggle that can be forgotten, Version 2.0 can allow user-configurable alarm timers for different conditions. A simple “snooze” feature to temporarily extends the timer during open scenarios could greatly improve user feedback.

<u>Motor Control Refinement</u>
The current motor logic is simple. Adding limit switches or encoders provides position feedback instead of assuming success. Current sensing could detects jams or obstructions. Soft-start/soft-stop PWM control can reduces stress and noise, and a maximum run-time would prevents motor damage during faults.
